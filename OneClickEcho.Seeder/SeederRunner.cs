...
public class SeederRunner
{
    public static async Task<bool> Execute(IServiceProvider app)
    {
        ...
        try
        {
            ...
            if (company is null)
            {
                companyRepository.Add(new Company("OneClick Echo", "bulkapi", "@p1Bulk"));
            }
            ...
            Domain.ApplicationUserAggregate.ApplicationUser? user = await userRepository.GetByEmailAsync("vesna.sibinovic@oneclick.rs");

            company = await companyRepository.GetByNameAsync("OneClick Echo");

            if (user is null && company is not null)
            {
                await userManager.CreateUser("vesna.sibinovic@oneclick.rs", "Testing123!", company.Id, UserRole.Administrator);
            }
