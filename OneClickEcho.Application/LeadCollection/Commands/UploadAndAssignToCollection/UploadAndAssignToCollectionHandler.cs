        ...
        UploadAndAssignLeadsToCollectionResponse>
{
    ...
    public async Task<Result<UploadAndAssignLeadsToCollectionResponse>> Handle(UploadAndAssignLeadsToCollectionCommand request, CancellationToken cancellationToken)
    {
        ...
        List<string> csvPhoneNumbers = records
            .Select(x => PhoneNumberHelper.Standardize(x.PhoneNumber))
            .Distinct()
            .ToList();

        List<Domain.LeadAggregate.Lead> duplicateLeads =
            await _leadRepository.GetAllByPhoneNumbersAndCompanyIdAsync(csvPhoneNumbers,
                CompanyId.Create(leadCollection.CompanyId.Value), cancellationToken);
        
        List<string> duplicatePhoneNumbers = duplicateLeads
            .Select(x => x.PhoneNumber)
            .Distinct()
            .ToList();
        
        var newRecords = records
            .Where(l => !duplicatePhoneNumbers.Contains(PhoneNumberHelper.Standardize(l.PhoneNumber)))
