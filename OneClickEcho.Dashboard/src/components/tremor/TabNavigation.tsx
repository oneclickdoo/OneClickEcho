...
type TabNavigationLinkProps = Omit<
...
>(({ asChild, disabled, className, children, ...props }, forwardedRef) => (
    <NavigationMenuPrimitives.Item className="flex" aria-disabled={disabled}>
        <NavigationMenuPrimitives.Link
            ...
            // Radix Link expects onSelect, ali nama nije potreban (držimo backward-compatible behavior)
            onSelect={() => { }}
            asChild={asChild}
            {...props}
        >
            {getSubtree({ asChild, children }, (subtreeChildren) => (
                <span
                    className={cx(
                        // base
