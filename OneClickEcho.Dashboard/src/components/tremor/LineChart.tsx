    ...
    return (
        <li
            className={cx(
                ...
                hasOnValueChange
                    ? "bg-transparent cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800"
                    : "cursor-default"
            )}
            onClick={(e) => {
                e.stopPropagation();
...
const Legend = React.forwardRef<HTMLOListElement, LegendProps>((props, ref) => {
    ...
    const [isKeyDowned, setIsKeyDowned] = React.useState<"ArrowLeft" | "ArrowRight" | null>(null);
    const intervalRef = React.useRef<ReturnType<typeof setInterval> | null>(null);
    ...
    React.useEffect(() => {
        const keyDownHandler = (key: "ArrowLeft" | "ArrowRight") => {
            scrollToTest(key === "ArrowLeft" ? "left" : "right");
        };
    ...
    React.useEffect(() => {
        const scrollable = scrollableRef.current;
        if (!scrollable || !enableLegendSlider) return;
        ...
        <ol ref={ref} className={cx("relative overflow-hidden", className)} {...other}>
            <li className="list-none">
                <div
                    ...
                    onScroll={enableLegendSlider ? checkScroll : undefined}
                >
                    <ul className="flex flex-wrap items-center gap-1" role="list">
                        {categories.map((category, index) => (
                            <LegendItem
                                ...
                                onClick={onClickLegendItem ? (c, col) => onClickLegendItem(c, col) : undefined}
                    ...
                    return (
                        <ChartTooltipRow
                            ...
                            color={getColorClassName(
                                categoryColors.get(p.name) as AvailableChartColorsKeys,
                                "bg"
                            )}
                        />
                    );
...
const LineChart = React.forwardRef<HTMLDivElement, LineChartProps>((props, ref) => {
    ...
    function onDotClick(itemData: any, event: React.MouseEvent) {
        ...
        const isSameDot = itemData.index === activeDot?.index && itemData.dataKey === activeDot?.dataKey;
        const isSingleValueSeries =
            hasOnlyOneValueForKey(data, itemData.dataKey) && activeLegend && activeLegend === itemData.dataKey;

        if (isSameDot || isSingleValueSeries) {
    ...
    function onCategoryClick(dataKey: string) {
        ...
        const isSameCategory = dataKey === activeLegend && !activeDot;
        const isSingleValueSeries =
            hasOnlyOneValueForKey(data, dataKey) && activeDot && activeDot.dataKey === dataKey;

        if (isSameCategory || isSingleValueSeries) {
            setActiveLegend(undefined);
            onValueChange?.(null);
        } else {
                            ...
                            showTooltip
                                ? ({ active, payload, label }) => (
                                    <OverviewChartTooltip
                                        ...
                                        categoryColors={categoryColors as unknown as Map<string, string>}
                                    />
                                )
                                : () => null
                        }
                            ...
                            activeDot={(p: any) => {
                                ...
                                return (
                                    <Dot
                                        ...
                                        onClick={(_, event) => onDotClick(p, event)}
                                    />
                                );
                            }}
                            dot={(p: any) => {
                                const { stroke, strokeLinecap, strokeLinejoin, strokeWidth, cx: cxCoord, cy: cyCoord, dataKey, index } = p;
