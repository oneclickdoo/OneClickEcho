...
const RightMeta = ({ hint, shortcut }: RightMetaProps) => {
    if (!hint && !shortcut) return null;

    return (
        <span className="ml-auto flex items-center gap-x-2 pl-2">
            {hint && <span className="text-sm text-gray-400 dark:text-gray-600">{hint}</span>}
            {shortcut && <span className="text-sm text-gray-400 dark:text-gray-600">{shortcut}</span>}
        </span>
    );
};
...
>(({ className, shortcut, hint, children, asChild: _asChild, ...props }, forwardedRef) => {
    return (
        ...
        >
            {children}
            <RightMeta hint={hint} shortcut={shortcut} />
        </DropdownMenuPrimitives.Item>
    );
});
...
>(({ className, hint, shortcut, children, checked, asChild: _asChild, ...props }, forwardedRef) => {
    return (
        ...
        >
            ...
            {/* Right meta (hint + shortcut) */}
            {(hint || shortcut) && (
                <span className="ml-auto flex items-center gap-x-2">
                    {hint && (
                        <span className="text-sm font-normal text-gray-400 dark:text-gray-600">
                            {hint}
                        </span>
                    )}
                    {shortcut && (
                        <span
                            className={cx(
                                "text-sm font-normal tracking-widest text-gray-400 dark:border-gray-800 dark:text-gray-600"
                            )}
                        >
                            {shortcut}
                        </span>
