...
function ListItem({
    ...
    reorderAriaLabel: (label: string) => string;
}) {
    ...
    return (
        <React.Fragment>
            <div ref={ref} className="relative border-b border-transparent">
                <div className={cx("relative flex items-center justify-between gap-2", draggableState.type === "dragging" && "opacity-50")}>
                    ...
                    <Button
                        ...
                        ref={dragHandleRef}
                        aria-label={reorderAriaLabel(item.label)}
                    >
            ...
            <div className="flex justify-center">
                <Popover>
                    <PopoverTrigger asChild>
                        <Button variant="secondary" className={cx("ml-auto hidden gap-x-2 px-2 py-1.5 text-sm sm:text-xs lg:flex")}>
                    ...
                    <PopoverContent align="end" sideOffset={7} className="z-50 w-fit space-y-2">
                        <Label className="font-medium">{t("displayProperties")}</Label>
                                    ...
                                    return (
                                        <div key={column.id} className={cx(!column.getCanHide() && "hidden")}>
                                            <ListItem
                                                ...
                                                item={item}
                                                index={index}
                                                reorderAriaLabel={(label) => t("reorder", { label })}
                                            />
                                        </div>
                                    );
                                })}
