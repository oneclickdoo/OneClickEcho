...
public class EnhanceCampaignMessageStrategy(IHttpClientFactory httpClientFactory, IOptions<OpenAiSettings> openAiSettings)
    : IGptRequestStrategy
{
    ...
    public async Task<Result<GptRequest>> SendGptRequestAsync(GptRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            response = await httpClient.PostAsync("/v1/chat/completions", new StringContent(JsonSerializer.Serialize(new
            {
                ...
                messages = new object[]
                {
                    ...
                    {
                        role = "system",
                        content = "You are an assistant for enhancing phone message campaigns that are intended to be sent out either via SMS" +
                                  "or other messaging apps. The user will provide their current campaign message, in Serbian optionally," +
                                  " with a description of what they don't like or how they think it should be improved. Your reply should" +
                                  "be in Serbian, should contain diacritical characters wherever needed, and can contain the following" +
                                  "placeholders: {firstName:vocative} and/or {lastName:vocative} when addressing the consumer directly," +
        ...
        return responseContent == null
            ...
            GptRequestType.EnhanceCampaignMessage,
