    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        foreach (Domain.CampaignAggregate.Campaign campaign in response.Value.UpcomingCampaigns)
        {
            ...
            ITrigger sendCampaignMessagesTrigger = TriggerBuilder.Create()
                ...
                .StartAt(new DateTimeOffset(campaign.SendingDatetime)) // Returned campaigns will always have a SendingDateTime set
            ...
            await _mediator.Send(new EnqueueCampaignCommand(campaign.Id.Value));

            await context.Scheduler.ScheduleJob(sendCampaignMessagesJob, sendCampaignMessagesTrigger);

            Console.WriteLine($"Successfully scheduled upcoming campaign: {campaign.Id.Value} - {campaign.Name} for {campaign.SendingDatetime.ToLocalTime()}");
        }
    }
}